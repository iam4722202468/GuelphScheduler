...
var clientSession = cs('asdf123'); //secret key
...
function doAlgorithm(DBID, SessionID, filterID, req, callback_) {
	...
	if(currentlyInUse.indexOf(req.csession['ID']) > -1) //don't let user spam refresh button (uses a lot of cpu...)
	{
	...
	{
		currentlyInUse.push(req.csession['ID'])
		
		PythonShell.run('./python/algorithm.py', options, function(err, returnData) {
			if (err) throw err;
			
			if(filterID != "") {
				var index = currentlyInUse.indexOf(req.csession['ID'])
	...
	filter.createFilterDBID(req.body.criteria, req.body.rateData, function(filterID) {
		sessionManager.generateSchedules(req.body.SessionID, function(data) {
			if(currentSessions[req.body.SessionID] === undefined)
				...
				doAlgorithm(currentSessions[req.body.SessionID]['DBID'], req.csession['ID'], filterID, req, function(a) {
					res.send(a);
				});
			}
		});
...
app.post('/', function(req,res) {
	...
	if(Object.keys(req.body).length > 29)
		...
		var options = {
			args: [req.csession['ID'], JSON.stringify(argsArray)]
		};
